...
export const getUserSchemaId = (value: UserType): string => {
...
export const addSchemaIdToUserType2: definitions.AdjustFunction<definitions.ContextParams, unknown, string> = ({
  value,
}) => {
  if (!isUserType(value)) {
...
export const addSchemaIdToUserType: definitions.AdjustFunction = ({ value }) => {
  if (!isUserType(value)) {
    throw new Error('Invalid enrty for user type')
  }
  const schemaId = getUserSchemaId(value)
