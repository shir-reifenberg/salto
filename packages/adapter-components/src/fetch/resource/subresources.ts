          ...
          Object.entries(def.recurseInto ?? {})
            ...
            .map(async ([fieldName, recurseDef]) => {
              const nestedRequestContext = _.mapValues(recurseDef.context.args, contextDef => {
                const transformer = createValueTransformer({ ...contextDef.transformation, single: true }) // transformer should only return one string value
                const requestContext = transformer(item)
                return _.get(requestContext, 'value')
              })
