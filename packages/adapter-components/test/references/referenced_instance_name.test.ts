...
import { addReferencesToInstanceNames } from '../../src/references/referenced_instance_name'
...
describe('referenced instances', () => {
  describe('addReferencesToInstanceNames', () => {
    ...
    // it('should sort instances in the right order', () => {
    //   const instances = elements.slice(0, -3).filter(isInstanceElement)
    //   const instancesToIdFields: InstanceIdFields[] = instances
    //     .map(instance => {
    //       if (instance.elemID.typeName === 'book') {
    //         return ({
    //           instance,
    //           idFields: getConfigWithDefault(
    //             transformationConfigByType.book,
    ...
    //         instance,
    //         idFields: getConfigWithDefault(
    //           transformationConfigByType.recipe,
    //           transformationDefaultConfig
    //         ).idFields,
    //       })
    //     })
    //   const orderedInstances = sortInstancesByReference(instancesToIdFields)
    //   expect(orderedInstances[0].instance.elemID).toEqual(instances[3].elemID)
    //   expect(orderedInstances[1].instance.elemID).toEqual(instances[0].elemID)
    //   expect(orderedInstances[2].instance.elemID).toEqual(instances[2].elemID)
    //   expect(orderedInstances[3].instance.elemID).toEqual(instances[1].elemID)
    // })

    // it('should throw an error due to a cycle of references', () => {
    //   const cycleElems = elements.slice(-5, -2)
    //   const instancesToIdFields: InstanceIdFields[] = cycleElems
    ...
    //         ).idFields,
    //       }))
    //   expect(() => sortInstancesByReference(instancesToIdFields))
    //     .toThrow(new Error(
    //              'Instances names cannot be created when a cycle of references is detected'
    //              ))
    // })
